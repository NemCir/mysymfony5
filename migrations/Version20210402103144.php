 ...
 */
final class Version20210402103144 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__airport AS SELECT id, airport_id, name, country, iata, icao, latitude, longitude, altitude, timezone, dst, tz, type, source FROM airport');
        $this->addSql('DROP TABLE airport');
        $this->addSql('CREATE TABLE airport (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, city_name VARCHAR(255) DEFAULT NULL, airport_id INTEGER NOT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, country VARCHAR(255) NOT NULL COLLATE BINARY, iata VARCHAR(3) DEFAULT NULL COLLATE BINARY, icao VARCHAR(4) DEFAULT NULL COLLATE BINARY, latitude NUMERIC(9, 6) NOT NULL, longitude NUMERIC(9, 6) NOT NULL, altitude INTEGER NOT NULL, timezone INTEGER NOT NULL, dst VARCHAR(1) NOT NULL COLLATE BINARY, tz VARCHAR(50) NOT NULL COLLATE BINARY, type VARCHAR(25) NOT NULL COLLATE BINARY, source VARCHAR(25) NOT NULL COLLATE BINARY, CONSTRAINT FK_7E91F7C2BD6D436E FOREIGN KEY (city_name) REFERENCES city (name) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO airport (id, airport_id, name, country, iata, icao, latitude, longitude, altitude, timezone, dst, tz, type, source) SELECT id, airport_id, name, country, iata, icao, latitude, longitude, altitude, timezone, dst, tz, type, source FROM __temp__airport');
